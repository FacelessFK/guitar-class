 ...
 *
 * JPEG کانال آلفا ندارد و PNG شفاف بدون این، سیاه درمی‌آید. همان
 * `--color-violet-surface` است — یعنی دقیقاً همان سطحی که آواتار در
 * سراسر پلتفرم رویش می‌نشیند — تا لبه‌ی عکس در صفحه گم شود.
 *
 * ⚠️ تنها رنگ خامِ خارج از `globals.css` در کل پروژه، و ناچار: بوم
 * `canvas` مقدار عددی می‌خواهد و به متغیر CSS دسترسی ندارد. با عوض شدن
 * جهت بصری این هم باید دستی عوض شود.
 */
const MATTE = "#24223a";
