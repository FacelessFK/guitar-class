...
const HOOK_KEY = "hook:jitsi:last-event";

/** سی روز — فقط برای اینکه کلید تا ابد نماند. */
const HOOK_TTL_MS = 30 * 24 * HOUR_MS;

/**
 * آخرین باری که سرور جیتسی رویدادی فرستاده.
 *
 * ⚠️ **این ضربان نیست و آستانه‌ی کهنگی ندارد**، برخلاف پنج جاروی بالا.
 * جارو هر دقیقه بی‌قید و شرط اجرا می‌شود، پس سکوتش خرابی است. هوک فقط
 * وقتی حرف می‌زند که کسی وارد اتاقی شود — و «امروز هیچ کلاسی نبود» یک
 * حالت کاملاً سالم است. آستانه گذاشتن رویش یعنی هشداری که هر شب روشن
 * می‌شود، و هشداری که همیشه روشن است دیده نمی‌شود.
 *
 * پس این عدد فقط گزارش می‌شود و در `status` اثری ندارد. چیزی که واقعاً
 * وزن دارد، تصمیمِ **به ازای هر جلسه** است: جلسه‌ای که هیچ رویداد
 * سرورتأییدی ندارد، تأییدنشده حساب می‌شود و پولش جابه‌جا نمی‌شود.
 *
 * تنها استفاده‌ی عملی‌اش لحظه‌ی استقرار است: بعد از اولین کلاس آزمایشی
 * باید غیر از `null` باشد. اگر نبود، ماژول prosody نصب یا فعال نیست.
 */
export async function recordHookEvent(at: Date = new Date()): Promise<void> {
  await redis.set(HOOK_KEY, at.toISOString(), "PX", HOOK_TTL_MS);
}

export async function readHookLastEventAt(): Promise<string | null> {
  return redis.get(HOOK_KEY);
}
