...
>
> ⚠️ **آداپتور S3 در برابر باکت واقعی آزموده نشده.** سیاست CORS باکت،
> پذیرش `UNSIGNED-PAYLOAD` و شکل دقیق میزبان چیزهایی‌اند که فقط سرویس
> واقعی نشان می‌دهد. اولین کار پس از ساختن باکت همین است. تا آن موقع
> آداپتور حافظه‌ای، آپلود و دانلود را خودِ API انجام می‌دهد
> (`/api/media/dev/...`) تا کل حلقه بدون حساب ابری اجرا شود — همان نقشی
> که درگاه جعلی برای پرداخت دارد. در تولید، ناقص بودن تنظیمات باعث
> می‌شود برنامه عمداً بالا نیاید.
>
