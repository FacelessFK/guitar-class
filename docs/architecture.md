...
### ۶.۲.۱ دو تنانت روی یک سرور

همین سرور جیتسی را تیم توسعه هم برای جلسات داخلی‌اش استفاده می‌کند.
بستن ورود ناشناس، آن‌ها را قفل کرد. راه‌حل، تنانت است نه باز کردن دوباره‌ی
ورود مهمان:

| تنانت | آدرس | ادعای `sub` | `room` |
|---|---|---|---|
| کلاس‌ها | `/lessons/<booking.room_id>` | `lessons` | همان `room_id` |
| تیم | `/team/<هر نامی>` | `team` | `*` |

با `JWT_ENABLE_DOMAIN_VERIFICATION=true`، ادعای `sub` باید با نام تنانت
در آدرس بخواند. نتیجه: **توکن تیم روی مسیر کلاس‌ها کار نمی‌کند، حتی با
`room: "*"`.** این با مرورگر واقعی تست شد — روی اتاق کلاسِ *باز*، توکن
تیم «Sorry, you're not allowed to join this call» گرفت. جهت معکوس هم
بسته است.
...
توکن تیم با `mint-team-token.sh` روی سرور ساخته می‌شود. سکرت مشترک است،
پس این اسکریپت نباید در اختیار تیم قرار بگیرد؛ فقط توکنِ ساخته‌شده.
