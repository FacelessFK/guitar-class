...
> **گواهی TLS هنوز نصب نشده.** Let's Encrypt از این سرور قابل دسترس
> نیست (تحریم) — `curl` به `acme-v02.api.letsencrypt.org` صفر برمی‌گرداند
> در حالی که گیتهاب باز است. گواهی فعلیِ `class.hyggemode.com` خودامضاست
> و مرورگر هشدار می‌دهد. تا وقتی گواهی واقعی نصب نشود، این نمونه فقط
> برای توسعه قابل استفاده است.
